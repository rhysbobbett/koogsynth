...
function playSound(note) {
  if (!activeNotes[note]) {
    ...
    activeNotes[note] = { vcaOscillator, gainNode, lfoMod, lfoModGain };
...
function stopSound(note) {
  if (activeNotes[note]) {
    const { vcaOscillator, gainNode, lfoMod } = activeNotes[note];
    vcaOscillator.stop();
    lfoMod.stop();
    gainNode.disconnect();
    lfoMod.disconnect();
...
vcaGainSlider.addEventListener('input', function () {
  ...
  for (const note in activeNotes) {
    activeNotes[note].gainNode.gain.setValueAtTime(gainValue, audioContext.currentTime);
  }
});
...
lfoFrequencySlider.addEventListener('input', function () {
  ...
  for (const note in activeNotes) {
    activeNotes[note].lfoMod.frequency.setValueAtTime(frequencyValue, audioContext.currentTime);
  }
});
...
lfoIntensitySlider.addEventListener('input', function () {
  ...
  for (const note in activeNotes) {
    activeNotes[note].lfoModGain.gain.setValueAtTime(intensityValue, audioContext.currentTime);
  }
});
